...
public class RayMarching implements IGameLogic{
	...
	private final float MAX_DIST = 10f;
	private final float SURFACE_DIST = 0.01f;
	
	private float sphereWidth = 0.02f;
	private Vec3 sphere = new Vec3(0f, 1f, 4);
	private float sphereDx = 0, sphereDy = 0;
	private Vec3 box = new Vec3(0.02f, 0.25f, 0.25f);
	...
	public float getDist(Vec3 p) {
		...
		if((box1Dist < SURFACE_DIST && sphereDist < SURFACE_DIST) || (box2Dist < SURFACE_DIST && sphereDist < SURFACE_DIST)) this.collision = true;
	...
	public void reset() {
		sphere = new Vec3(0f, 1f, 4);
		Random rand = new Random();
		sphereDy = (rand.nextFloat()*2 - 1)/100;
		sphereDx = 0.01f * (rand.nextInt(2)*2 - 1);
	}
	...
	public void update(float interval) {
		...
		sphere.x += sphereDx;
		sphere.y += sphereDy;
		if(sphere.y - 0.02 <= -1.0 || sphere.y + 0.02 >= 3.0f) {
			sphereDy *= -1;
		}
		if(sphere.x - 0.02 <= -2.0) {
			score2++;
			this.reset();
		}
		if(sphere.x + 0.02 >= 2.0) {
	...
	public void render(Graphics2D g2d) {
		// TODO Auto-generated method stub
		CenteredString cscore1 = new CenteredString(g2d, String.valueOf(score1), 0, 0, GameEngine.displayWidth/2, GameEngine.displayHeight/3, new Font("SansSerif", Font.PLAIN, 20));
		CenteredString cscore2 = new CenteredString(g2d, String.valueOf(score2), GameEngine.displayWidth/2, 0, GameEngine.displayWidth/2, GameEngine.displayHeight/3, new Font("SansSerif", Font.PLAIN, 20));
