...
public class Pipeline {
	...
	public void setMode(int mode) {
		assert(mode == TRIANGLES || mode == WIREFRAME);
		this.mode = mode;
	}
	...
	public void draw(Graphics3D g3d, IndexedLineList<Vertex> lineList) {
		this.g3d = g3d;
		processVertices(lineList.vertices, lineList.indices);
	}
	...
	private void processVertices(ArrayList<Vertex> vertices, ArrayList<Integer> indices) {
		...
		for (Vertex v : vertices) {
			vOut.add(new Vertex(rotation._multiply(v.pos)._add(translation)));
		}
		
		if(mode == TRIANGLES) assembleTriangles(vOut, indices);
		else if(mode == WIREFRAME) drawWireframe(vOut, indices);
	}
	...
	private void drawWireframe(ArrayList<Vertex> vertices, ArrayList<Integer> indices) {
		for (Vertex v : vertices) {
			st.transform(v);
		}
		for (int i = 0; i < indices.size(); i+= 2) {
			g3d.drawLine(vertices.get(indices.get(i)).pos, vertices.get(indices.get(i+1)).pos, PackedColor.Black);
		}
	}
