...
public class Ball extends PongMovable {
	private int radius, speed;
	
	public Ball(int dx, int dy, int x, int y, Color col, int radius, int speed) {
		super(dx, dy, x, y, col);
	...
	public Ball(Color col, int radius, int speed) {
		super(col);
		this.radius = radius;
		this.speed = speed;
	}
	...
	public void collision(Paddle p) {
		int cType = Collision.sideRectangleCircleIntersection(p.getX(), p.getY(), p.getWidth(), p.getHeight(), this.x, this.y, this.radius, this.dx, this.dy);
		if (cType == 0 || cType == 2) {
			this.translate(-this.dx, -this.dy);
			this.dx *= -1;
