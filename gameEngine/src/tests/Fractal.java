...
public class Fractal implements IGameLogic {
		...
		if(gameState == 4) {
			...
			if(!zoomState) {
				...
				if(GameEngine.keyboard.keysPressed[KeyEvent.VK_Z]) {
					double mX = Maths.map(GameEngine.mouse.mouseX, 0, GameEngine.displayWidth-1, this.view[0], this.view[1]);
					double mY = Maths.map(GameEngine.mouse.mouseY, 0, GameEngine.displayHeight-1, this.view[3], this.view[2]);
	...
	public void render(Graphics2D g2d) {
		...
		if (gameState == 0) {
			for (CenteredString cs: text) {
				cs.render(g2d, Color.white);
			}		
		//Squares
		} else if (gameState == 1) {
			Rectangle r1 = new Rectangle(250, 150, 300, 300);
			drawSquares(g2d, r1, 0);
		//PerpendicularLines
		} else if (gameState == 2) {
			perpendicularLines(g2d, 400, 300, 400, 0, 0);
		//Tree
		} else if (gameState == 3) {
			tree(g2d, GameEngine.displayWidth/2, GameEngine.displayHeight, 100, 90, 0);
		//SerpinskiCarpet
		} else if (gameState == 5) {
			Rectangle r1 = new Rectangle(300, 200, 200, 200);
			serpinskiCarpet(g2d, r1, 0);
		//SerpinskiGasket (Triangle)
		} else if (gameState == 6) {
			Triangle<Vec2> t = new Triangle<Vec2>(new Vec2(150, 525), new Vec2(650, 525), new Vec2(400, 75));
			serpinskiGasket(g2d, t, 0);
		//Cantor Set
		} else if(gameState == 7 ) {
			cantorSet(g2d, 100, 100, 600, 0);
		//KochSnowflake
		} else if(gameState == 8 ) {
			float sL = 400;
			float t = (float) (410 - sL*SIN_60);
			kochSnowflake(g2d, new Vec2(sL/2, 410), new Vec2(3*sL/2, 410), 0);
			kochSnowflake(g2d, new Vec2(3*sL/2, 410), new Vec2(sL, t), 0);
			kochSnowflake(g2d, new Vec2(sL, t), new Vec2(sL/2, 410), 0);
		}
	}
	...
	public void render3D(Graphics3D g3d) {
		...
		if(gameState == 4) {
			...
			g3d.drawPixels(this.mandelbrot);
		}
